...
from .core.pseudosr.rrdb import RRDBNet
from .core.cyclegan.models import GeneratorResNet, Discriminator
from .core.pseudosr.vggs import VGGFeatureExtractor
...
class PseudoModelESRGAN(PseudoModel):
    def __init__(self, config, **kwargs):
        super().__init__(config, **kwargs)
        self.cfg = config

        self.idt_input_clean = True # corrupted (False) or clean (True)
        self.U = RRDBNet(3, 3, scale_factor=self.scale_factor).to(self.device)
        
        self.D_sr = NLayerDiscriminator(3, scale_factor=1, norm_layer=nn.InstanceNorm2d)
        self.opt_U = optim.Adam(self.U.parameters(), lr=config.lr, betas=(0.9, 0.999))
        self.opt_Dsr = optim.Adam(self.D_sr.parameters(), lr=config.lr, betas=(0.9, 0.999))
        self.lr_U = optim.lr_scheduler.MultiStepLR(self.opt_U, milestones=config.lr_milestones, gamma=0.5)
        self.lr_Dsr = optim.lr_scheduler.MultiStepLR(self.opt_Dsr, milestones=config.lr_milestones, gamma=0.5)
        self.nets["U"] = self.U
        self.nets["D_sr"] = self.D_sr
        self.optims["U"] = self.opt_U
        self.optims["D_sr"] = self.opt_Dsr
        self.lr_decays["U"] = self.lr_U
        self.lr_decays["D_sr"] = self.lr_Dsr
        self.discs.append("D_sr")

        self.ragan_loss = GANLoss("vanilla")
        self.vgg_feat = "conv5_4"
        self.vgg = VGGFeatureExtractor([self.vgg_feat], use_input_norm=True, range_norm=False)

        self.sr_warmup_iter = kwargs.get('warmup_iters', 500)
        self.sr_pix_weight = 0.01
        self.sr_gp_weight = 1e-6
        self.sr_vgg_weight = 1
        self.sr_gan_weight = 0.005
        self.sr_gp_weight = 1e-6

    def warmup_checker(self):
        return self.global_step <= self.sr_warmup_iter
    ...
    def train_step(self, Ys, Xs, Yds, Zs):
        ...
        # loss_geo = self.l1_loss(fake_Yds, geo_Yds)
        loss_d_sr = self.gan_loss(pred_sr_y, True, False)
        loss_total_gen = loss_gan_Gyx + loss_gan_Gxy + self.cyc_weight * loss_cycle + self.idt_weight * loss_idt_Gxy + self.d_sr_weight * loss_d_sr
        loss_dict["G_xy_gan"] = loss_gan_Gxy.item()
        loss_dict["G_xy_idt"] = loss_idt_Gxy.item()
        loss_dict["cyc_loss"] = loss_cycle.item()
        # loss_dict["G_xy_geo"] = loss_geo.item()
        ...
        # U
        if self.warmup_checker():
            fake_sr = self.U(rec_Yds.detach())

            # D
            self.net_grad_toggle(["D_sr"], True)
            self.opt_Dsr.zero_grad()
            fake_pred = self.D_sr(fake_sr).detach()
            real_pred = self.D_sr(Ys)
            ...
            fake_loss.backward()
            self.opt_Dsr.step()
            loss_dict["D_sr"] = real_loss.item() + fake_loss.item()

            # G
            self.net_grad_toggle(["D_sr"], False)
            self.opt_U.zero_grad()
            loss_pix = self.l1_loss(fake_sr, Ys)
            loss_vgg = self.l1_loss(self.vgg(fake_sr)[self.vgg_feat], self.vgg(Ys)[self.vgg_feat].detach())

            real_pred = self.D_sr(Ys).detach()
            fake_pred = self.D_sr(fake_sr)
            ...
            loss_U = self.sr_pix_weight * loss_pix + self.sr_gp_weight * loss_U_gp + self.sr_vgg_weight * loss_vgg + self.sr_gan_weight * loss_gan
            loss_U.backward()
            self.opt_U.step()
